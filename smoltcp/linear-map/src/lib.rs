//! Based on heapless/linear_map with allocator-api2/Vec
//!
//! There is also an official linear-map crate on crates.io but that implement a bit different logic
//!
//! So it seemed that it was the simplest method for implementing linear-map (with 'heaped' Vec) for smoltcp
#![no_std]
...
pub struct LinearMap<K, V, const N: usize> {
    ...
    pub(crate) buffer: crate::Vec<(K, V)>,
}
