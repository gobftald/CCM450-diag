...
impl InterfaceInner {
    ...
    pub(super) fn process_ipv4<'a>(
        ...
        #[cfg(feature = "socket-dhcpv4")]
        {
                ...
                && matches!(self.caps.medium, Medium::Ethernet)
            {
                    ...
                    .find_map(|i| Dhcpv4Socket::downcast_mut(&mut i.socket))
                {
                    ...
                    // This way we avoid validating the UDP checksum twice for all non-DHCP UDP packets
                    // (one here, one in `process_udp`)
        ...
        match ipv4_repr.next_header {
            ...
            IpProtocol::Udp => {
                self.process_udp(
                    sockets, meta, /*handled_by_raw_socket,*/ ip_repr, ip_payload,
                )
            }
