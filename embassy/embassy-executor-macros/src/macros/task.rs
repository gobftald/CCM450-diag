...
pub fn run(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if !f.sig.generics.where_clause.is_none() {
        error(
            &mut errors,
            &f.sig,
            "task functions must not have `where` clauses",
        );
    }
    if !f.sig.abi.is_none() {
        error(
            &mut errors,
            &f.sig,
            "task functions must not have an ABI qualifier",
        );
    }
        ...
        match arg {
            syn::FnArg::Receiver(_) => {
                error(
                    &mut errors,
                    arg,
                    "task functions must not have `self` arguments",
                );
            }
            syn::FnArg::Typed(t) => {
    ...
    let result = quote! {
        ...
        #visibility fn #task_ident #generics (#fargs) -> #embassy_executor::SpawnToken #where_clause{
                #task_outer_body
        }
